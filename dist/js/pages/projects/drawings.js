...
$(function() {
    ...
    $.post(url + "api/projects/get-drawings-files/"+id , {
        ...
        if(data.success == true){
            data.project.forEach(function (value) {
               ...
               $('#drawings-form').append( '                    <section class="hero">\n' +
    ...
    swal.fire(
        {
            ...
            preConfirm: function () {
                ...
                reader.onloadend = function (ev) {
                    ...
                    $.post(url + "api/projects/add-drawing-file/" + id, {
                        token: getCookie("token"),
                        ext: ext,
                        key: key,
                        file: base64
...
function removeFile(key , file , id) {
    swal.fire({
        ...
        if (result.value && result.isConfirmed) {
            $.post(url + "api/projects/delete-drawing-file/" + id, {
                token: getCookie("token"),
                file: file,
                key:key
            }).done(function (data) {
